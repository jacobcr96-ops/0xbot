...
## Retired: calibration_shadow (do not consume)
- Historical `calibration_shadow` / `shadow_only` decision JSONs may still exist on this branch or in git history — **they are retired**.
- Consumers **MUST ignore** any decision whose `risk_flags` contain any of: `calibration_shadow` | `shadow_only` | `pipe_check` | `PIPECHECK`.
- Also ignore late/calibration markers: `post_move_not_early` (late must never be treated as a live BUY).
- Going forward, Grok publishes **only real pursue→BUY** intents (no invented shadow stubs). Publish script skips shadow/pipe_check files.

## Real BUY emit policy
- Age+MC auto-pursue alone is **not** enough to publish a BUY.
- Require at least one of: (a) organic X evidence (not boost-only), (b) first-buyer/flow positive, (c) explicit multi-channel intel pass (e.g. S1/S3).
- Thin Dex-new / paid boost / parasite-only → reject, no emit.
- When `XINTEL_ARMED=true`, new emits stamp `do_not_execute_until_armed=false`. 0xbot still has its own arm gate.
